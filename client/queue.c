...
int		write_to_client(t_conf *conf, t_simple_list *client, const char *buffer, int total_len)
{
	...
	//DPRINTF(1, "In [write_to_client] function. Client session_id 0x%x\n", client->session_id);
	// a case when pseudoclient got new connection from remote. There should be 2 bytes of new session id inside
	if (!socket_is_valid(client->fd_wo) && !conf->is_local_port_forwarding)
	{
		...
		DPRINTF(1, "New session_id is %d\n", new_session_id);
		if (connect_socket(conf, &tsock))
		{
			MYERROR("Could not connect to host\n");
			tsock = -1;
		}
		if (add_rpf_client(conf, tsock, tsock, new_session_id))
			MYERROR("add_rpf_client\n");
